import { None, Option, OptionSomeType, Result, Some } from "../src";
import { SomeImpl } from "../src/option";
import { eq } from "./util";
...
test("map / andThen", () => {
  ...
  const mapped = (someString as Option<string>).andThen((val) =>
    Option.Some(!!val),
  );
...
test("all / any", () => {
  ...
  const options = [
    Option.Some("foo" as const),
    Option.Some("bar" as const),
    Option.Some("baz" as const),
  ] as const;
...
test("to string", () => {
  ...
  expect(`${Option.Some({ name: "George" })}`).toEqual(
    'Some({"name":"George"})',
  );
  expect(`${None}`).toEqual("None");
});
